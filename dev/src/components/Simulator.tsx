...
import { renderAnimation, drawPet, PET_Y_WALK } from '@src/browser-bundle';
...
const ctrl: React.CSSProperties = {
  ...
  padding: '4px 6px', fontFamily: 'monospace', fontSize: 12,
};
...
export default function Simulator() {
  ...
  useEffect(() => { liveRef.current = { iconVal, temp, night, speed }; }, [iconVal, temp, night, speed]);
  ...
  useEffect(() => {
    ...
    function loop(ts: number) {
      ...
      const snap   = { temperature: temp, ...parseWeather(iconVal, night) };
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      ...
      <canvas ref={canvasRef} width={32 * SCALE} height={32 * SCALE}
        style={{ imageRendering: 'pixelated', border: '1px solid #444', background: '#000' }} />
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center', maxWidth: 420 }}>
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: '#888' }}>
          Temp (°C)
          <input type="number" value={temp} min={-30} max={50} style={{ ...ctrl, width: 64 }}
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: '#888' }}>
          ...
          <input type="range" min={0.25} max={4} step={0.25} value={speed}
            onChange={e => setSpeed(Number(e.target.value))} style={{ width: 110 }} />
        </label>
