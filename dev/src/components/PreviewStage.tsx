...
async function postWeather(iconVal: string, temp: number, night: boolean, humidity: number, windSpeed: number): Promise<void> {
  const { weatherCode, isDay } = parseWeather(iconVal, night);
  await fetch('/api/control/weather', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ weatherCode, isDay, temperature: temp, humidity, windSpeed, windDirection: 0 }),
  });
}
...
export default function PreviewStage() {
  ...
  const liveRef = useRef({ iconVal, temp, humidity, windSpeed, night, speed });
  useEffect(() => {
    liveRef.current = { iconVal, temp, humidity, windSpeed, night, speed };
  }, [iconVal, temp, humidity, windSpeed, night, speed]);
  ...
  useEffect(() => {
    ...
    function loop(ts: number) {
      ...
      const { iconVal, temp, humidity, windSpeed, night, speed } = liveRef.current;
      const snap = {
        temperature: temp,
        humidity,
        windSpeed,
  ...
  const handleIconVal = useCallback(
    (val: string) => {
      setIconVal(val);
      if (remote) void postWeather(val, temp, night, humidity, windSpeed);
    },
    [remote, temp, night, humidity, windSpeed],
  );

  const handleTemp = useCallback(
    (val: number) => {
      setTemp(val);
      if (remote) void postWeather(iconVal, val, night, humidity, windSpeed);
    },
    [remote, iconVal, night, humidity, windSpeed],
  );

  const handleNight = useCallback(
    (val: boolean) => {
      setNight(val);
      if (remote) void postWeather(iconVal, temp, val, humidity, windSpeed);
    },
    [remote, iconVal, temp, humidity, windSpeed],
  );

  const handleHumidity = useCallback(
    (val: number) => {
      setHumidity(val);
      if (remote) void postWeather(iconVal, temp, night, val, windSpeed);
    },
    [remote, iconVal, temp, night, windSpeed],
  );

  const handleWindSpeed = useCallback(
    (val: number) => {
      setWindSpeed(val);
      if (remote) void postWeather(iconVal, temp, night, humidity, val);
    },
    [remote, iconVal, temp, night, humidity],
  );
  ...
  return (
    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <h1 style={{ margin: 0, fontSize: 11, letterSpacing: 2, color: '#666' }}>PREVIEW</h1>
        {remote &&
      ...
      {remote && liveState ? (
        <div style={{ fontSize: 10, color: '#4a8', letterSpacing: 1, textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 10, color: '#555', minHeight: 14, textAlign: 'center' }}>{info}</div>
      )}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center', width: '100%' }}>
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: '#888' }}>
          Humidity {humidity}%
          <input type="range" min={0} max={100} step={1} value={humidity} style={{ width: 90 }} onChange={(e) => handleHumidity(Number(e.target.value))} />
        </label>

        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: '#888' }}>
          Wind {windSpeed} km/h
          <input type="range" min={0} max={60} step={1} value={windSpeed} style={{ width: 90 }} onChange={(e) => handleWindSpeed(Number(e.target.value))} />
        </label>

        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: '#888' }}>
          Night mode
          <input type="checkbox" checked={night} onChange={(e) => handleNight(e.target.checked)} style={{ marginTop: 6 }} />
        </label>
        ...
        {!remote && (
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, color: '#888' }}>
            ...
            <input type="range" min={0.25} max={4} step={0.25} value={speed} onChange={(e) => setSpeed(Number(e.target.value))} style={{ width: 90 }} />
          </label>
        )}
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 11, color: '#888', width: '100%' }}>
        <span style={{ color: '#555', letterSpacing: 1, fontSize: 10 }}>FORCE BEHAVIOR {remote && <span style={{ color: '#4a8' }}>(→ matrix)</span>}</span>
