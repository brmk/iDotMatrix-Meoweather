...
| 4 | [[phase-4-studio-and-palette-editor]] | ✅ | 3 | `transform/phase-4-studio-and-palette-editor` | 2026-06-06 | `useCustomization` hook owns fetch/save/reset. `PaletteEditor` component: reserved `o g s l r` locked, user swatches removable with used-in-sprites guard, auto-char from `a–z`. Studio wired to API; no more `/save-sprites`/`/save-pet-config` in UI. Version badge in header. No frontend test runner added (covered by backend Phase-3 contract tests + manual verify). Char-assignment strategy: next free `a–z` excluding reserved+used. |
| 5 | [[phase-5-ui-restructure]] | ✅ | 4 | `transform/phase-5-ui-restructure` | 2026-06-06 | `Simulator.tsx` deleted (absorbed into `PreviewStage.tsx` + `DevicePanel.tsx`). 3 zones: Device / Studio / Diagnostics. `PreviewStage` always-visible; weather+behavior controls deduplicated. `Connection.tsx` accepts optional `matrixPaused` prop to avoid duplicate SSE. |
